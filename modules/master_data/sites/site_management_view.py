...
from modules.master_data.sites.site_dialog import SiteDialog
from modules.master_data.sites.site_service import SiteService


class SiteManagementView(ctk.CTkFrame):
    """
    Site Master Management workspace.

    Final implementation includes:
    - Complete view layout
    - Permission-aware toolbar
    - SiteService-backed loading
    - Search and status filtering
    - Create Site workflow
    - Edit Site workflow
    - Archive Site workflow
    - Restore Site workflow
    ...
    def __init__(
        ...
        self.selected_site: dict[str, Any] | None = None
        self._search_after_id: str | None = None
        self._is_loading = False

        self.can_view = PermissionService.can_view_sites(
            self.user
        )
        self.can_create = PermissionService.can_create_sites(
            self.user
        )
        self.can_edit = PermissionService.can_edit_sites(
            self.user
        )
        self.can_archive = PermissionService.can_archive_sites(
            self.user
        )
        self.can_restore = PermissionService.can_restore_sites(
            self.user
        )
    ...
    def _build_header(self) -> None:
        ...
        ctk.CTkLabel(
            title_block,
            text=(
                "Create and maintain Site Master records "
    ...
    def _build_toolbar(self) -> None:
        self.toolbar = MasterDataToolbar(
            self,
            search_placeholder=(
                "Search site code, site name, project, or client."
            ),
    ...
    def _build_grid(self) -> None:
        ...
        self.grid_view = MasterDataGrid(
            ...
            columns=columns,
            id_key="id",
            empty_message="No Site Master records found.",
            result_label_singular="site",
            result_label_plural="sites",
    ...
    def _handle_grid_selection(
        ...
        self.toolbar.set_selected_record(site)
    ...
    def _handle_new(self) -> None:
        ...
        SiteDialog(
            ...
            mode=SiteDialog.MODE_CREATE,
            on_save=self._create_site,
        )
    ...
    def _handle_edit(self) -> None:
        ...
        if not bool(site.get("is_active")):
        ...
        SiteDialog(
            self,
            current_user=self.user,
            mode=SiteDialog.MODE_EDIT,
            site=site,
            on_save=lambda data: self._update_site(
                site.get("id"),
                data,
            ),
        )
    ...
    def _create_site(
        ...
        try:
            created = SiteService.create_site(
                data,
                current_user=self.user,
            )

            self.after_idle(
                lambda record=created: self._after_site_saved(
                    record,
                    action="created",
                )
            )
    ...
    def _update_site(
        ...
        if not site_id:
            raise ValueError(
                "The selected Site Master record has no ID."
            )

        self._set_busy(True)

        try:
            updated = SiteService.update_site(
                site_id,
                data,
                current_user=self.user,
            )
            ...
            self.after_idle(
                lambda record=updated: self._after_site_saved(
                    record,
                    action="updated",
                )
            )
    ...
    def _after_site_saved(
        self,
        site: dict[str, Any],
        *,
        action: str,
    ) -> None:
        prepared_site = self._prepare_site_row(site)
        self.selected_site = prepared_site
        ...
        messagebox.showinfo(
            "Site Saved",
            (
                f"Site Master record "
                f"{self._site_label(site)} was "
    ...
    def _handle_archive(self) -> None:
        ...
        if not bool(site.get("is_active")):
            messagebox.showwarning(
                ...
                (
                    f"Site Master record "
                    f"{self._site_label(site)} is already inactive."
                ),
                parent=self.winfo_toplevel(),
            )
            return
        ...
        confirmed = messagebox.askyesno(
            "Archive Site",
            (
                f"Archive Site Master record "
                f"{self._site_label(site)}?\n\n"
                "The site will become inactive but will remain "
        ...
        try:
            archived = SiteService.deactivate_site(
                site.get("id"),
                current_user=self.user,
            )

            self._after_site_status_changed(
                archived,
                action="archived",
            )
            ...
            messagebox.showerror(
                "Unable to Archive Site",
                (
                    "The Site Master record could not be archived."
                    f"\n\n{exc}"
                ),
                parent=self.winfo_toplevel(),
            )
    ...
    def _handle_restore(self) -> None:
        ...
        if bool(site.get("is_active")):
            messagebox.showwarning(
                "Already Active",
                (
                    f"Site Master record "
                    f"{self._site_label(site)} is already active."
                ),
                parent=self.winfo_toplevel(),
            )
            return
        ...
        confirmed = messagebox.askyesno(
            "Restore Site",
            (
                f"Restore Site Master record "
                f"{self._site_label(site)}?\n\n"
                "The site's linked Project Master record must "
                "also be active."
            ),
            parent=self.winfo_toplevel(),
        )
        ...
        try:
            restored = SiteService.restore_site(
                site.get("id"),
                current_user=self.user,
            )
            ...
            messagebox.showerror(
                "Unable to Restore Site",
                (
                    "The Site Master record could not be restored."
                    f"\n\n{exc}"
                ),
                parent=self.winfo_toplevel(),
            )
    ...
    def _after_site_status_changed(
        self,
        site: dict[str, Any],
        *,
        action: str,
        ...
        # load_sites() clears the selection automatically.
        messagebox.showinfo(
            "Site Updated",
            (
                f"Site Master record "
    ...
    def load_sites(
        ...
        *,
        ...
        selected_site_id: Any | None = None,
    ) -> None:
        if self._is_loading:
            return
        ...
        try:
            if selected_site_id is None and preserve_selection:
                selected_site_id = self._get_selected_site_id()

            search_text = self._get_toolbar_search_text()
            status_filter = self._get_toolbar_status_filter()

            self.sites = SiteService.search(
            ...
            messagebox.showerror(
                ...
                (
                    "The Site Master records could not be loaded.\n\n"
                    f"{exc}"
                ),
                parent=self,
            )
    ...
    def _restore_selection(
        ...
        if callable(select_method):
            select_method(site_id)

        self.selected_site = matching_site
        self.toolbar.set_selected_record(
            matching_site
        )

    def _get_selected_site_id(self) -> Any | None:
        if not self.selected_site:
            return None

        return self.selected_site.get("id")
    ...
    def _show_selection_warning(
        ...
        messagebox.showwarning(
            "Select a Site",
            (
                "Select a Site Master record before "
    ...
    def _site_label(
        ...
        label = " - ".join(
            value
            for value in (site_code, site_name)
            if value
        )

        return label or "the selected site"
