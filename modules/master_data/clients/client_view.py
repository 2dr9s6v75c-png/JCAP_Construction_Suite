...
class ClientManagementView(ctk.CTkFrame):
    """
    Client Master Management workspace.

    Final Client Management view.

    Includes:
    - Service-backed loading, search, and status filtering
    - Create and Edit workflows
    - Archive and Restore workflows
    - Permission-aware UI actions
    - Confirmation and success messages
    - Selection and refresh state management
    """
    ...
    def __init__(
        ...
        self.can_view = PermissionService.can_view_clients(
            self.user
        )
        self.can_create = PermissionService.can_create_clients(
            self.user
        )
        self.can_edit = PermissionService.can_edit_clients(
            self.user
        )
        self.can_archive = PermissionService.can_archive_clients(
            self.user
        )
        self.can_restore = PermissionService.can_restore_clients(
            self.user
        )
    ...
    def _build_header(self) -> None:
        ...
        ctk.CTkLabel(
            ...
            text=(
                ...
                "used throughout the JCAP Construction Suite."
            ),
            font=("Segoe UI", 13),
    ...
    def _build_toolbar(self) -> None:
        self.toolbar = MasterDataToolbar(
            self,
            search_placeholder=(
                "Search client code, name, contact person, "
                "phone, email..."
            ),
    ...
    def _handle_edit(self) -> None:
        ...
        if not client:
            messagebox.showwarning(
                "Select a Client",
                (
                    "Select a Client Master record before "
                    "clicking Edit."
                ),
                parent=self.winfo_toplevel(),
            )
        ...
        ClientDialog(
            ...
            client=client,
            on_save=lambda data: self._update_client(
                client.get("id"),
                data,
            ),
        )
    ...
    def _create_client(
        ...
        try:
            ...
            self.after_idle(
                lambda record=created: self._after_client_saved(
                    record,
                    action="created",
                )
            )
    ...
    def _update_client(
        ...
        if not client_id:
            raise ValueError(
                "The selected Client Master record has no ID."
            )
        ...
        try:
            ...
            self.after_idle(
                lambda record=updated: self._after_client_saved(
                    record,
                    action="updated",
                )
            )
    ...
    def _after_client_saved(
        self,
        client: dict[str, Any],
        *,
        action: str,
    ) -> None:
        self.selected_client = self._prepare_client_row(
            client
        )
        ...
        record_label = " - ".join(
            value
            for value in (client_code, client_name)
            if value
        )
        ...
        messagebox.showinfo(
            "Client Saved",
            (
                f"Client Master record {record_label} was "
    ...
    def _handle_archive(self) -> None:
        ...
        if not client:
            messagebox.showwarning(
                "Select a Client",
                (
                    "Select an active Client Master record "
        ...
        if not bool(client.get("is_active")):
            messagebox.showwarning(
                "Already Inactive",
                (
                    "The selected Client Master record is "
                    "already inactive."
                ),
                parent=self.winfo_toplevel(),
            )
        ...
        if not client_id:
            messagebox.showerror(
                "Archive Failed",
                (
                    "The selected Client Master record "
        ...
        client_label = self._get_client_label(client)

        confirmed = messagebox.askyesno(
            "Archive Client",
            (
                f'Archive client "{client_label}"?\n\n'
                "The client will be removed from the Active list "
        ...
        try:
            ...
            messagebox.showerror(
                "Archive Failed",
                (
                    "The Client Master record could not be archived."
                    f"\n\nDetails: {exc}"
                ),
                parent=self.winfo_toplevel(),
            )
        ...
        messagebox.showinfo(
            "Client Archived",
            (
                f'Client "{self._get_client_label(archived)}" '
    ...
    def _handle_restore(self) -> None:
        ...
        if not client:
            messagebox.showwarning(
                "Select a Client",
                (
                    "Select an inactive Client Master record "
        ...
        confirmed = messagebox.askyesno(
            "Restore Client",
            (
                f'Restore client "{client_label}"?\n\n'
                "The client will become available in the "
                "Active list again."
            ),
            parent=self.winfo_toplevel(),
        )
        ...
        try:
            ...
            messagebox.showerror(
                "Restore Failed",
                (
                    "The Client Master record could not be restored."
                    f"\n\nDetails: {exc}"
                ),
                parent=self.winfo_toplevel(),
            )
        ...
        messagebox.showinfo(
            "Client Restored",
            (
                f'Client "{self._get_client_label(restored)}" '
    ...
    def load_clients(
        ...
        *,
        ...
        try:
            ...
            self.clients = [
                self._prepare_client_row(client)
    ...
    def _build_empty_message(
        *,
        ...
        if search_text:
            return (
                f'No {status_filter.lower()} clients match '
                f'"{search_text}".'
            )
